	...
	public <T> OdmResult update(T pojo, boolean updateChilds)
		...
		try {
			...
			try {
				...
				// Need to include objectClass for add operation
				List<OdmAttribute> attributesWithObjectClass = odmpojo.getAttributes(pojo, true);
				odmStack.add(dn, attributesWithObjectClass);
				added = true;
			}
			if (statisticsEnabled) {
				s = System.currentTimeMillis() - s;
